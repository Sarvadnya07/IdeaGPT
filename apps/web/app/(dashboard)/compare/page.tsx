...
export default function ComparePage() {
  ...
  const [comparisonResult, setComparisonResult] =
    useState<IdeaComparisonResponse | null>(null);
  ...
  const handleCompare = async () => {
    ...
    try {
      const res = await api.post<IdeaComparisonResponse>(
        "/evaluations/compare",
        {
      ...
      const detail =
        err?.response?.data?.detail || "Failed to compare selected ideas.";
      setErrorMsg(detail);
    } finally {
  ...
  const highestScoreIdea = useMemo(() => {
    if (!comparisonResult?.ideas || comparisonResult.ideas.length === 0)
      return null;
    return [...comparisonResult.ideas].sort(
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-neutral-800 pb-6">
        <div>
          ...
          <p className="text-neutral-400 text-sm mt-1">
            Side-by-side multi-dimensional benchmarking and competitive gap
            analysis.
          </p>
        </div>
      ...
      {projectsQuery.isLoading ? (
        ...
        <div className="bg-neutral-900/50 border border-neutral-800 rounded-xl p-12 text-center max-w-lg mx-auto my-12 space-y-4">
          ...
          <p className="text-neutral-400 text-sm">
            You need at least one project with ideas to start comparing startup
            concepts.
          </p>
          <Link
            href="/projects/new"
        ...
        <div className="bg-neutral-900/50 border border-neutral-800 rounded-xl p-12 text-center max-w-lg mx-auto my-12 space-y-4">
          ...
          <h3 className="text-xl font-semibold text-white">
            No Ideas in Selected Project
          </h3>
          <p className="text-neutral-400 text-sm">
            Add at least two ideas to this project to perform multi-dimensional
            comparison.
          </p>
          <Link
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-neutral-400 mt-0.5">
                  Choose between 2 and 5 ideas from this project.
                </p>
              </div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <h3 className="text-2xl font-bold text-white">
                        {highestScoreIdea.title}
                      </h3>
                      <p className="text-xs text-neutral-400 mt-0.5">
                        Achieved top score of{" "}
                        <span className="text-indigo-300 font-semibold">
                          {highestScoreIdea.overall_score}/100
                        </span>{" "}
                    ...
                    {item.evaluation_status === "unevaluated" && (
                      <div className="bg-neutral-950 border border-neutral-800 rounded-lg p-3 text-center space-y-2">
                        <p className="text-xs text-neutral-400">
                          Evaluation not available for this idea.
                        </p>
                        <Link
              ...
              <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-6 space-y-4 overflow-x-auto">
                ...
                <table className="w-full text-left text-sm text-neutral-300">
                  ...
                  <tbody className="divide-y divide-neutral-800/60">
                    {Object.entries(comparisonResult.dimension_labels).map(
                      ([key, label]) => (
