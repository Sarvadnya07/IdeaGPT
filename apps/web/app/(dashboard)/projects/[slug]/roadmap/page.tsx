...
export default function ProjectRoadmapPage({
  ...
  const { roadmapsQuery, createRoadmap, updateRoadmap } = useRoadmaps(
    project?.id || null,
  );
  ...
  const handleToggleTaskStatus = async (
    ...
    const updatedMilestones = JSON.parse(
      JSON.stringify(currentRoadmap.milestones),
    ) as Milestone[];
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-neutral-800 pb-4">
        <div>
          ...
          <p className="text-neutral-400 text-xs mt-0.5">
            Interactive milestone planning and task execution tracker. Click any
            task to toggle status.
          </p>
        </div>
      ...
      {projectsQuery.isLoading || roadmapsQuery.isLoading ? (
        ...
        <div className="bg-neutral-900/50 border border-neutral-800 rounded-xl p-10 text-center max-w-md mx-auto my-8 space-y-4">
          ...
          <p className="text-neutral-400 text-xs leading-relaxed">
            Generate an AI-synthesized execution roadmap with customized phases,
            objectives, and task trackers.
          </p>
        ...
        <div className="space-y-6">
          {currentRoadmap.milestones.map((m, idx) => {
            const completedCount = m.tasks.filter(
              (t) => t.status === "completed",
            ).length;
            const progressPercent =
              m.tasks.length > 0
                ? Math.round((completedCount / m.tasks.length) * 100)
                : 0;

            return (
              <div
                key={idx}
                className="bg-neutral-900/90 border border-neutral-800 rounded-xl p-5 space-y-4"
              >
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] bg-neutral-800 text-neutral-400 px-2 py-0.5 rounded font-mono">
                        {completedCount}/{m.tasks.length} Completed (
                        {progressPercent}%)
                      </span>
                    </div>
                  ...
                  {m.tasks.map((task, tIdx) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div
                            className={`font-medium transition-colors ${task.status === "completed" ? "text-neutral-400 line-through" : "text-neutral-200"}`}
                          >
                            {task.title}
                          </div>
                          {task.description && (
