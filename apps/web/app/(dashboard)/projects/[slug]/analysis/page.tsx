...
export default function AnalysisPage({
  ...
  const activeEval =
    evaluationQuery.data || (jobId ? null : latestCompletedEval);

  const insightsQuery = useInsights(
    activeEval?.status === "COMPLETED" ? activeEval?.id || null : null,
  );
  ...
  if (
    ...
    projectEvaluationsQuery.isLoading
  ) {
    return (
  ...
  const handleSubmit = async () => {
    try {
      ...
      // 1. Save Idea
      const savedIdea = await saveIdea.mutateAsync({
  ...
  if (!isEditingIdea && (jobId || jobStatus)) {
      ...
      return (
        <div className="flex flex-col items-center justify-center py-32 space-y-6">
          ...
          <div className="flex items-center gap-2 text-zinc-400 text-sm">
            ...
            {jobStatus === "QUEUED"
              ? "Waiting in queue..."
              : "Processing neural evaluation via Groq..."}
          </div>
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
          <div className="flex flex-col md:flex-row justify-between md:items-center bg-[#0b0b0d] border border-zinc-800 p-6 rounded-2xl gap-4">
            <div>
              ...
              <p className="text-zinc-400 text-sm">
                Here is the architectural and strategic breakdown of your idea.
              </p>
            </div>
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col gap-2">
                <button
                  ...
                  className="px-3 py-1.5 bg-zinc-900 border border-zinc-800 text-zinc-300 hover:text-white rounded-lg text-xs font-semibold transition-colors"
                >
                  Download Markdown
                </button>
                <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="bg-[#0b0b0d] border border-zinc-800/80 p-6 rounded-2xl space-y-4">
              <h3 className="text-sm font-bold text-zinc-400 tracking-wider uppercase mb-2">
          ...
          <div className="bg-[#0b0b0d] border border-zinc-800 p-6 rounded-2xl space-y-4">
            ...
            <div className="prose prose-invert max-w-none text-sm text-zinc-300">
              <ReactMarkdown>
                {result.architecture_breakdown || "No breakdown provided."}
              </ReactMarkdown>
            </div>
          </div>
          ...
          <div className="bg-[#0b0b0d]/60 border border-zinc-800/40 p-4 rounded-xl text-xs text-zinc-500 flex flex-wrap gap-x-8 gap-y-2">
            <div>
              <strong>Provider:</strong> {metadata.provider || "N/A"}
            </div>
            <div>
              <strong>Model:</strong> {metadata.model || "N/A"}
            </div>
            <div>
            ...
            <div>
              ...
              {metadata.duration_ms ? `${metadata.duration_ms}ms` : "N/A"}
            </div>
            <div>
              <strong>Cached:</strong> {metadata.cached ? "Yes" : "No"}
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            {insights && (
              <>
                ...
                >
                  <div className="grid grid-cols-3 gap-4 mb-4">
                    {[
                      ...
                      {
                        label: "SAM",
                        value: insights.market_analysis.sam.split("—")[0],
                      },
                      {
                        label: "SOM",
                        value: insights.market_analysis.som.split("—")[0],
                      },
                    ].map((m) => (
                      <div
                        key={m.label}
                  ...
                  <div className="text-[11px] text-zinc-400 space-y-1.5">
                    <div>
                      <span className="text-zinc-500 font-bold">Maturity:</span>{" "}
                      {insights.market_analysis.market_maturity}
                    </div>
                    <div>
                      <span className="text-zinc-500 font-bold">Barriers:</span>{" "}
                      {insights.market_analysis.adoption_barriers.join(", ")}
                    </div>
                  </div>
                </InsightModule>
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {insights.competitor_analysis.competitive_advantages.map(
                          (a, i) => (
                            <span
                              key={i}
                              className="text-[10px] bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded-full font-semibold"
                            >
                              {a}
                            </span>
                          ),
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {insights.competitor_analysis.competitive_gaps.map(
                          (g, i) => (
                            <span
                              key={i}
                              className="text-[10px] bg-orange-500/10 border border-orange-500/20 text-orange-400 px-2 py-0.5 rounded-full font-semibold"
                            >
                              {g}
                            </span>
                          ),
                ...
                >
                  <div className="grid grid-cols-2 gap-4 text-[11px] text-zinc-400">
                    <div>
                      ...
                      {insights.technical_feasibility.engineering_complexity}
                    </div>
                    <div>
                      <span className="text-zinc-500 font-bold block mb-0.5">
                        Timeline
                      </span>
                      {insights.technical_feasibility.development_timeline}
                    </div>
                    <div className="col-span-2">
                      <span className="text-zinc-500 font-bold block mb-0.5">
                        Infrastructure
                      </span>
                      {insights.technical_feasibility.infrastructure}
                    </div>
                  </div>
                </InsightModule>
                ...
                >
                  <div className="space-y-2 text-[11px] text-zinc-400">
                    <div>
                      ...
                      {insights.business_model.revenue_model}
                    </div>
                    <div>
                      <span className="text-zinc-500 font-bold">Pricing:</span>{" "}
                      {insights.business_model.pricing}
                    </div>
                    <div>
                      <span className="text-zinc-500 font-bold">
                        Acquisition:
                      </span>{" "}
                      {insights.business_model.customer_acquisition}
                    </div>
                    <div className="flex gap-6 pt-1">
                      <div className="text-center">
                        <div className="text-xl font-black text-white">
                          {insights.business_model.viability_score}
                        </div>
                      ...
                      <div className="text-center">
                        <div className="text-xl font-black text-white">
                          {insights.business_model.scalability_score}
                        </div>
                        <div className="text-[9px] text-zinc-500 uppercase tracking-widest">
                          Scalability
                        </div>
                ...
                >
                  <div className="space-y-3">
                    {Object.entries(insights.risk_analysis).map(
                      ([key, risk]) => (
                        <div key={key} className="flex items-start gap-3">
                          <span
                            className={`text-[9px] font-bold px-2 py-0.5 rounded uppercase tracking-widest shrink-0 mt-0.5 ${
                              risk.level === "Low" ||
                ...
                >
                  <div className="space-y-4">
                    {(["quick_wins", "medium_term", "long_term"] as const).map(
                      (tier) => (
                        <div key={tier}>
                          <div className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest mb-2">
                            {tier === "quick_wins"
          ...
          <div className="flex justify-end">
            ...
            >
              <BrainCircuit className="w-4 h-4" /> Rerun Analysis with Different
              AI Model
            </button>
          </div>
        </div>
    ...
    if (jobStatus === "FAILED") {
      return (
        <div className="text-red-500 p-6 bg-red-500/10 rounded-xl border border-red-500/20">
    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-12">
      <div className="mb-8">
        ...
        <p className="text-zinc-500 text-sm">
          Provide details about your startup to receive an AI-generated
      ...
      <div className="flex items-center gap-2 mb-8 bg-[#0b0b0d] p-4 rounded-xl border border-zinc-800/60 overflow-x-auto">
        {[1, 2, 3].map((s) => (
          <React.Fragment key={s}>
            <div
              className={`flex items-center justify-center w-8 h-8 rounded-full text-sm font-bold transition-colors ${step >= s ? "bg-indigo-600 text-white" : "bg-zinc-800 text-zinc-500"}`}
            >
              {s}
            </div>
            {s < 3 && (
              <div
                className={`w-12 h-0.5 ${step > s ? "bg-indigo-600" : "bg-zinc-800"}`}
              />
            )}
          </React.Fragment>
        ))}
      ...
      <div className="bg-[#0b0b0d] border border-zinc-800/60 p-8 rounded-2xl shadow-xl">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, core_problem: e.target.value })
                  }
                  className="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-indigo-500 h-24 resize-none"
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      primary_platforms: e.target.value,
                    })
                  }
                  className="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-indigo-500"
                >
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      existing_tech_stack: e.target.value,
                    })
                  }
                  className="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-indigo-500"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      monetization_model: e.target.value,
                    })
                  }
                  className="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-indigo-500"
            ...
            <div className="space-y-4">
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      key_competitors: e.target.value,
                    })
                  }
                  className="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-indigo-500 h-24 resize-none"
              ...
              <div className="pt-4 border-t border-zinc-800/60">
                ...
                <p className="text-xs text-zinc-400 mb-3">
                  Choose the AI model used to perform deep strategic evaluation
                  and SWOT analysis.
                </p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  >
                    <div className="text-xs font-bold text-white flex items-center justify-between">
                      ...
                      <span className="text-[10px] bg-indigo-500/20 text-indigo-400 px-2 py-0.5 rounded font-mono font-bold">
                        Top Pick
                      </span>
                    </div>
                    <p className="text-[11px] text-zinc-400 mt-1">
                      120B parameter flagship reasoning & analysis
                    </p>
                  </button>
                  ...
                  >
                    ...
                    <p className="text-[11px] text-zinc-400 mt-1">
                      High-speed technical reasoning & architecture
                    </p>
                  </button>
                  ...
                  >
                    ...
                    <p className="text-[11px] text-zinc-400 mt-1">
                      Ultra-low latency sub-second evaluation
                    </p>
                  </button>
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      setSelectedProvider("deterministic");
                      setSelectedModel("rule-based-v2.6");
                    }}
                    className={`p-3.5 rounded-xl border text-left transition-all ${
                  ...
                  >
                    <div className="text-xs font-bold text-white flex items-center justify-between">
                      ...
                      <span className="text-[10px] bg-zinc-700 text-zinc-300 px-2 py-0.5 rounded font-mono font-bold">
                        0-Token
                      </span>
                    </div>
                    <p className="text-[11px] text-zinc-400 mt-1">
                      Instant offline scoring & heuristics
                    </p>
                  </button>
                </div>
              </div>
        ...
        <div className="flex justify-between mt-10 pt-6 border-t border-zinc-800/60">
          ...
          {step < 3 ? (
            ...
            >
              {saveIdea.isPending || triggerEvaluation.isPending ? (
                <Loader2 className="w-4 h-4 animate-spin" />
              ) : (
