    ...
    <div className="space-y-8 py-4 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-zinc-900 pb-6">
        <div className="space-y-2">
          ...
          <p className="text-xs text-zinc-500 max-w-2xl leading-relaxed">
            Institutional VC valuation ranges, investability scorecards, dilution cap table modeling,
            and funding stage roadmaps.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          >
            {isGenerating ? (
              <>
                <Loader2 className="w-3.5 h-3.5 animate-spin" /> Evaluating Pitch...
              </>
            ) : (
              <>
      ...
      {result ? (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="p-6 rounded-2xl bg-zinc-950/60 border border-zinc-900 space-y-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xs font-bold text-zinc-300 uppercase tracking-wider">Estimated Valuation</h3>
              </div>
              <div>
                <span className="text-2xl font-bold text-white block">
                  {formatCurrency(result.valuation_range.pre_money_min_usd)} - {formatCurrency(result.valuation_range.pre_money_max_usd)}
                </span>
                <span className="text-xs text-zinc-500 mt-1 block">
                  Target Raise: <strong className="text-zinc-300">{formatCurrency(result.valuation_range.target_raise_usd)}</strong> (~{result.valuation_range.dilution_pct}% Dilution)
                </span>
              </div>
            ...
            <div className="p-6 rounded-2xl bg-zinc-950/60 border border-zinc-900 md:col-span-2 space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-xs font-bold text-zinc-300 uppercase tracking-wider">VC Investability Scorecard</h3>
                </div>
                <span className="px-2.5 py-0.5 rounded-full text-xs font-bold bg-amber-500/20 text-amber-300">
                  {result.investor_scorecard.overall_investability} / 100 Overall
                </span>
              </div>

              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-2">
                <div className="p-3 rounded-xl bg-black/40 border border-zinc-900 text-center">
                  <span className="text-lg font-bold text-white block">{result.investor_scorecard.market_opportunity}</span>
                  <span className="text-[10px] text-zinc-500 uppercase font-semibold">Market TAM</span>
                </div>
                <div className="p-3 rounded-xl bg-black/40 border border-zinc-900 text-center">
                  <span className="text-lg font-bold text-white block">{result.investor_scorecard.team_and_execution}</span>
                  <span className="text-[10px] text-zinc-500 uppercase font-semibold">Execution</span>
                </div>
                <div className="p-3 rounded-xl bg-black/40 border border-zinc-900 text-center">
                  <span className="text-lg font-bold text-white block">{result.investor_scorecard.defensibility_moat}</span>
                  <span className="text-[10px] text-zinc-500 uppercase font-semibold">Defensibility</span>
                </div>
                <div className="p-3 rounded-xl bg-black/40 border border-zinc-900 text-center">
                  <span className="text-lg font-bold text-white block">{result.investor_scorecard.unit_economics}</span>
                  <span className="text-[10px] text-zinc-500 uppercase font-semibold">Unit Econ</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="p-6 rounded-2xl bg-zinc-950/60 border border-zinc-900 space-y-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xs font-bold text-zinc-300 uppercase tracking-wider">Milestone Funding Stages</h3>
              </div>
                ...
                {result.funding_stages.map((stage, idx) => (
                  <div key={idx} className="p-4 rounded-xl bg-black/40 border border-zinc-900 space-y-2">
                    <div className="flex justify-between items-center text-xs">
                      ...
                      <span className="font-mono text-emerald-400 text-[11px]">{stage.valuation_benchmark}</span>
                    </div>
                    <span className="text-[11px] text-zinc-400 block">Target ARR: <strong>{stage.target_arr}</strong></span>
            ...
            <div className="p-6 rounded-2xl bg-zinc-950/60 border border-zinc-900 space-y-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xs font-bold text-zinc-300 uppercase tracking-wider">Cap Table Dilution Simulation</h3>
              </div>
                ...
                <table className="w-full text-left text-xs">
                  ...
                  <tbody className="divide-y divide-zinc-900/60 text-zinc-300 font-mono">
                    {result.cap_table_simulation.map((c, idx) => (
                      <tr key={idx}>
                        <td className="py-2.5 font-sans font-medium text-white">{c.stakeholder}</td>
                        <td className="py-2.5">{c.initial_equity_pct}%</td>
                        <td className="py-2.5 text-amber-400">{c.post_seed_equity_pct}%</td>
                        <td className="py-2.5 text-indigo-400">{c.post_series_a_pct}%</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="p-6 rounded-2xl bg-zinc-950/60 border border-zinc-900 space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-xs font-bold text-zinc-300 uppercase tracking-wider">Due Diligence Risk Matrix</h3>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {result.risk_matrix.map((r, idx) => (
                <div key={idx} className="p-4 rounded-xl bg-black/40 border border-zinc-900 space-y-2">
                  <div className="flex justify-between items-center">
                    <span className="text-xs font-bold text-white">{r.risk_factor}</span>
                    <span
                  ...
                  <p className="text-[11px] text-zinc-400 leading-relaxed">{r.mitigation_strategy}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="flex flex-col items-center justify-center py-20 border border-zinc-900/60 rounded-2xl bg-[#0b0b0d] text-center p-8 space-y-4">
          ...
          <h3 className="text-lg font-bold text-white">No Investor Report Generated Yet</h3>
          <p className="text-xs text-zinc-500 max-w-md leading-relaxed">
            Click &quot;Generate VC Report&quot; to calculate institutional valuation ranges, dilution schedules,
            and due diligence risk assessments.
          </p>
        </div>
      )}
    </div>
