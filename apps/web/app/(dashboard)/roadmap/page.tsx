...
export default function RoadmapPage() {
  ...
  const { roadmapsQuery, createRoadmap, updateRoadmap, deleteRoadmap } =
    useRoadmaps(activeProjectId);
  ...
  const handleToggleTaskStatus = async (
    ...
    const updatedMilestones = JSON.parse(
      JSON.stringify(activeRoadmap.milestones),
    ) as Milestone[];
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-zinc-900 pb-6">
        <div className="space-y-1">
          ...
          <p className="text-xs text-zinc-500 max-w-2xl leading-relaxed">
            Track execution timelines, milestones, and task statuses across your
            projects.
          </p>
        </div>
      ...
      {projectsQuery.isLoading || roadmapsQuery.isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 border border-zinc-900/60 rounded-2xl bg-[#0b0b0d] text-center p-8 space-y-4">
          ...
          <p className="text-xs text-zinc-500 max-w-md">
            Create your first project from the dashboard before generating a
            product roadmap.
          </p>
        </div>
      ) : !activeRoadmap ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 border border-zinc-900/60 rounded-2xl bg-[#0b0b0d] text-center p-8 space-y-4">
          ...
          <h3 className="text-lg font-bold text-white">
            No Roadmap for {activeProject?.title}
          </h3>
          <p className="text-xs text-zinc-500 max-w-md leading-relaxed">
            Generate an AI-synthesized execution roadmap tailored specifically
            to your startup idea.
          </p>
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
            ...
            >
              {isGeneratingAI || createRoadmap.isPending ? (
                <>
                  <Loader2 className="w-4 h-4 animate-spin" /> Generating with
                  AI...
                </>
              ) : (
                <>
