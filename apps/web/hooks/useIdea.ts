...
export function normalizeIdeaPayload(
  ...
  if (data.target_audience)
    customMetadata.target_audience = data.target_audience;
  if (data.existing_tech_stack)
    customMetadata.existing_tech_stack = data.existing_tech_stack;
  if (data.primary_platforms)
    customMetadata.primary_platforms = data.primary_platforms;
  if (data.monetization_model)
    customMetadata.monetization_model = data.monetization_model;
  if (data.key_competitors)
    customMetadata.key_competitors = data.key_competitors;
  if (data.technical_risks)
    customMetadata.technical_risks = data.technical_risks;
  ...
  return {
    title:
      data.title ||
      (data.elevator_pitch
        ? data.elevator_pitch.slice(0, 50)
        : "Untitled Idea"),
    problem_statement: problem.length >= 10 ? problem : problem.padEnd(10, " "),
    solution_description:
      solution.length >= 10 ? solution : solution.padEnd(10, " "),
