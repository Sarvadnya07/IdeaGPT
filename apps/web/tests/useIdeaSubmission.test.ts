...
describe("Idea Payload Normalization (CQ-01)", () => {
  it("normalizes client-side form data into canonical backend schema structure", () => {
    const inputData: Partial<IdeaData> = {
      ...
      problem_statement:
        "Validating technical ideas takes weeks of manual work.",
      solution_description:
        "Automated feasibility evaluations and architectural blueprints.",
      target_audience: "Startup Founders & Solo Engineers",
      business_model: "Subscription SaaS",
      unique_selling_proposition:
        "Instant deterministic scoring and AI multi-model analysis",
    ...
    expect(normalized.problem_statement).toBe(
      "Validating technical ideas takes weeks of manual work.",
    );
    expect(normalized.solution_description).toBe(
      "Automated feasibility evaluations and architectural blueprints.",
    );
